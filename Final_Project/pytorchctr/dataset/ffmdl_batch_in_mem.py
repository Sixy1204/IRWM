...
class FFMDataset(Dataset):
    def __init__(self, dataset_path=None, data_prefix='tr', rebuild_cache=False, tr_field_dims=None, read_flag=0, neg_ratio=1.):
        '''
        test_flag: 
        ...
        self.neg_ratio = neg_ratio if neg_ratio >= 1. else 1.
    ...
    def __getitem__(self, idx):  # idx = 10*context_idx + pos
        if self.read_flag == 0:
            with self.env.begin(write=False) as txn:
                ...
                item_idxes = np.random.choice(item_idxes, int(item_array.shape[1]*self.neg_ratio), replace=True)
            ...
            with self.env.begin(write=False) as txn:
                ...
                item_idxes = np.random.choice(neg_item_idxes, int(item_array.shape[1]*self.neg_ratio), replace=False)
                items = self.items[item_idxes, :, :]
            ...
            with self.env.begin(write=False) as txn:
                item_array = self.item_arrays['citem_%d'%idx]
                ctx_array = self.ctx_arrays['ctx_%d'%idx]
                pos_item_idxes = item_array[0, :].astype(np.long)
                neg_item_idxes = np.setxor1d(pos_item_idxes, self.item_set, True)
                neg_item_idxes = np.random.choice(neg_item_idxes, int(item_array.shape[1]*self.neg_ratio), replace=False)
                item_idxes = np.hstack((pos_item_idxes, neg_item_idxes))
                items = self.items[item_idxes, :, :]
                flags = np.hstack((item_array[1, :], np.zeros_like(neg_item_idxes)))
            pos = np.zeros_like(flags)
        else:
        ...
        if self.read_flag == 0:
            ctx_array = np.tile(ctx_array, (items.shape[0], 1, 1))
...
if __name__ == '__main__':
    ...
    def main(dataset):
        ...
        device='cuda:0'
        data_loader = DataLoader(dataset, batch_size=1, num_workers=0, shuffle=False, collate_fn=collate_fn)
        ...
        #for i, (data_pack, imp_data_pack) in enumerate(zip(data_loader, imp_data_loader)):
            ...
            for j in range(target.shape[0]):
                hist = [int(t2[j, 1, k]) for k in range(t2.shape[-1]) if t2[j, 0, k] == 3]
            print(hist)
            print(target.reshape(-1, dataset.item_num)[-1, hist])
            sys.exit(0)
    ...
    dataset = FFMDataset(dataset_path=sys.argv[1], data_prefix='va', rebuild_cache=False, read_flag=1)
